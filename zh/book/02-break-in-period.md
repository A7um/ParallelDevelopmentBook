...
Cherny 公开的[月度数字](https://www.reddit.com/r/ClaudeAI/comments/1px44q0/claude_code_creator_boris_cherny_reports_a_full/)——30 天里 259 个 PR，497 次提交，加了 40k 行，删了 38k 行，全部由 Claude Code 生成，1.6k 个 session，约 325 亿 token——就是第四阶段从外面看的吞吐。这段数字里最有信息量的不是"加了多少"，而是"**删了多少**"。你不会不小心删 38k 行。你删，是因为在第四阶段你终于有吞吐去清理历史遗留，这是你交付新功能*之外*的活。
...
一条编码通用工程原则的 skill——比如"组合优于继承"——几乎没有用。Agent 本来就知道。一条编码"**在这个代码库里**，后台任务定义在 `jobs/*.ts`，必须在 `jobs/retries.ts` 里也注册对应的重试策略，Agent 一直会漏第二步"的 skill——这个才是金子，因为它针对的是你的伤疤组织。

这也是为什么把别人的 `AGENTS.md` 整份拿过来很少奏效。他们的伤疤不是你的。那让他们的工作流跑起来的、来之不易的四十行特定性，是他们代码库、他们工具、他们 Agent 犯过两次之后才写下来的那种错误所特有的。你可以偷他们 skill 文件的**形状**，内容必须是你的。
