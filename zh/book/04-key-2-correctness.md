...
对这些，Agent 可以扮演用户。现代 Agent 能驱动浏览器、截图、填表单、报告体验。诀窍是**让 Agent 扮演具体的用户角色**，因为不同角色看到的问题不一样：

- **新手用户**：只看屏幕上的东西。不知道什么是控制台。两次失败就放弃。写的报告像*"我点了保存，页面空白了 5 秒"*——不是*"init 失败，500"*。
- **熟手用户**：主动找捷径、试旁路、注意到缺少快捷键。
- **对抗用户**：输异常输入、灌长字符串、点错顺序、想办法把它弄崩。

每种角色暴露一类不同的 bug。一条"像用户一样测一下"的泛指令会塌成通用工程师口吻、什么都抓不到。强制 Agent **只拥有那个角色拥有的感官**才是让报告有用的关键。

*参考*：[`zero-review/auto-test`](https://github.com/A7um/zero-review/tree/main/skills/auto-test)
...
1. 需求 spec（来自第 3 章）
2. 从 spec 写的测试计划，按复杂度分层审查
3. 架构步（第 5 章）产出模块边界
4. Agent 写实现 + 测试体
5. Agent 跑全套、调试失败、直到全绿
6. Agent 对 UI 类工作做 Agent 当用户测试
7. 人审：
   - 测试计划覆盖度按复杂度分层
   - Agent 当用户的报告
   - 最终全绿状态

4–6 步无人值守。这就是全部意义。你的注意力花在第 2 步和第 7 步——框契约、确认契约关上了。
